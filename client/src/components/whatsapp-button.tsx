...
export function WhatsAppButton() {
    return (
        <motion.a
            ...
            target="_blank"
            rel="noreferrer"
            title="Chat with us on WhatsApp (+94 76 287 1658)"
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            whileHover={{ scale: 1.06 }}
            whileTap={{ scale: 0.94 }}
            className="fixed bottom-6 left-6 z-50 flex items-center gap-2.5 bg-[#25D366] hover:bg-[#20ba5a] text-white px-4 py-3 rounded-full shadow-[0_10px_25px_rgba(37,211,102,0.4)] transition-all duration-300 group font-semibold text-xs border border-emerald-400/30 select-none cursor-pointer"
        >
            ...
            <span className="tracking-wide">WhatsApp with Us</span>
        </motion.a>
    );
}
