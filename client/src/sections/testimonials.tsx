        ...
        <section className="py-20 md:py-40 px-4 md:px-16 lg:px-24 xl:px-32 w-full bg-gray-50 overflow-hidden">
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-2 justify-start items-start">
                ...
                <div className="lg:col-span-5 flex flex-col items-start mt-20">
                    ...
                    <motion.h2 className="text-3xl md:text-[34px]/12 text-zinc-900 mt-5 leading-tight font-medium max-w-100"
                        initial={{ y: 50, opacity: 0 }}
                        whileInView={{ y: 0, opacity: 1 }}
                        viewport={{ once: true }}
                        transition={{ type: "spring", stiffness: 240, damping: 70, mass: 1 }}
                    >
                        Trusted by Teams. Proven by Products.
                    </motion.h2>

                    <motion.p className="text-zinc-500 text-sm md:text-base mt-2.5 max-w-85"
                        initial={{ y: 50, opacity: 0 }}
                        whileInView={{ y: 0, opacity: 1 }}
                        viewport={{ once: true }}
                        transition={{ delay: 0.2, type: "spring", stiffness: 320, damping: 70, mass: 1 }}
                    >
                        Honest words from the clients we've built for — startups, scale-ups, and enterprises alike.
                    </motion.p>
                </div>
